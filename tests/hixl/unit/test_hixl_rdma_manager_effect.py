#!/usr/bin/env python3
"""
Test: does spawning RdmaManagerActor break HIXL connect?
Same as test_hixl_dynamic_ports.py but with _ensure_init_rdma_manager() called
BEFORE HIXL init.
"""
import asyncio
import os
...
COORD = "/tmp/hixl_rdma_manager_effect"
NBYTES = 64
...
class Producer(Actor):
    def __init__(self):
        ...
        # KEY: Call _ensure_init_rdma_manager BEFORE HIXL init
        print(f"[Producer] Spawning RdmaManagerActor...", flush=True)
        from monarch._src.rdma.rdma import _ensure_init_rdma_manager
        _ensure_init_rdma_manager().block_on()
        print(f"[Producer] RdmaManagerActor spawned OK", flush=True)
        ...
        print(f"[Producer] dev={dev} addr={hex(self.addr)} engine={self.engine_id}", flush=True)
...
async def main():
    print("=" * 60)
    print("Test: RdmaManagerActor spawn effect on HIXL")
    print("=" * 60, flush=True)
    try:
        os.unlink(COORD)
    except Exception:
